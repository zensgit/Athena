...
## Addendum (2026-06-02, later — correction): Q2b shipped broken, then fixed at `cf9ce3a`

The "Backend-Verify CI green" claim in the addendum above was **premature** — it gated on the Backend Verify *job*, not the run conclusion. `d1dd45b` / `675f43e` / `f52b130` were red on the RM-notification full-stack E2E gate. Two compounding bugs the unit suite could not catch:

1. **The resolver's throw poisoned the writer's transaction.** `TenantContextResolverService` was `@Transactional(readOnly = true)` and threw `TargetFolderTenantException` on reject. Thrown across its proxy it marked the writer's `REQUIRES_NEW` transaction rollback-only *before* the writer's catch could persist a clean FAILED row → `UnexpectedRollbackException` at commit → `processedCount=0`. A Mockito resolver cannot mark a real transaction rollback-only, so every unit test passed. Fixed (`afbc017`) by making `resolveTenantForTargetFolder` **return** a `TenantResolution` (RESOLVED / NO_TENANT_SYSTEM / UNRESOLVED) instead of throwing; writers branch on the result. Guarded by `TenantResolverTransactionIsolationTest`, a `@DataJpaTest` that reproduces the poison with teeth.

2. **The migration-seeded systemDefault tenant has a null root.** Migration `059` seeds a `default` tenant (`is_enabled=true`, `is_system_default=true`, `rootNodeId=null`). So "an enabled tenant exists" was always true while the parent-chain walk could never match it → every root-folder write in a single-tenant deployment became UNRESOLVED → FAILED. Fixed (`cf9ce3a`) by keying the existence check on a non-null root (`existsByDeletedFalseAndEnabledTrueAndRootNodeIdNotNull`): only a real tenant (which always has a root) forces a reject; with just the systemDefault tenant the resolver returns NO_TENANT_SYSTEM and writers write untenanted (legacy single-tenant behaviour).

Q2b's reject contract above stands; the mechanism is now return-based, not throw-based. **Closed and full-run CI green at `cf9ce3a` (run 26821803696 — 7/7, including the RM-notification gate), 2026-06-02.** Process lesson: gate closure on the run conclusion (`gh run view <run> --json conclusion`), never a single job.
