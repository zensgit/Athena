...
- Current deployed bundle observed in the latest recheck: `main.a10e6709.js`, **3,286,459 bytes** uncompressed / **818,653 bytes** gzip. Source map removed (`*.map` = 0 in the container). The hash changes on each frontend rebuild; resolve it from `index.html` before future timing tests.
...
```
JS_PATH=$(curl -ks https://23.254.236.11/ \
  | grep -Eo '/static/js/main\.[^"]+\.js' \
  | head -1)
echo "$JS_PATH"
curl -o /dev/null -k -w 'time=%{time_total}s speed=%{speed_download}B/s size=%{size_download}\n' \
  "https://23.254.236.11${JS_PATH}"
```
