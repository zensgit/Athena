# P4 PR-55 RM Activity Contributor Family Highlights Design

## Goal

Add a backend-only RM analytics highlights surface for contributor-family breakdowns, mirroring the recent-window comparison semantics already used by contributor highlights and family highlights.

## Scope

- add `GET /api/v1/records/activity-contributor-family-highlights`
- JSON only
...
- contributor `limit` reuses the contributor-family-report limit semantics
- default `5`, clamp `1..50`
...
Nested family rows are built from the union of current-window and previous-window families for each contributor, filtered to rows with non-zero activity in either window.
...
- reuses `aggregateContributorFamilyReportFamilies(...)` and `mergeContributorFamilyReportFamilies(...)` for nested family breakdowns
- sorts contributors by max(current, previous), then current count, then label
- reuses the contributor-family-report limit normalization instead of introducing a new limit family
