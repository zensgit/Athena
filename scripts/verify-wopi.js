...
const CLEANUP_AFTER = /^1|true$/i.test(process.env.ECM_VERIFY_CLEANUP || '');
...
const waitForSearchMatch = async (request, apiBase, token, query, preferredName) => {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const results = await fetchSearchResults(request, apiBase, token, query);
    const match = findDocument(results, preferredName);
    if (match) {
      return match;
    }
    await sleep(2000);
...
(async () => {
  ...
  try {
    ...
    if (!documentMatch) {
      const delayedMatch = await waitForSearchMatch(
        page.request,
        apiBase,
        apiToken,
        FILE_QUERY,
    ...
    if (!documentMatch && !FILE_NAME_OVERRIDE) {
      ...
      cleanupContext = { id: sampleDocument.id, name: sampleDocument.name, apiBase, apiToken };
    ...
    if (CLEANUP_AFTER && cleanupContext?.id) {
      log(`Cleanup: removing ${cleanupContext.name} (${cleanupContext.id})`);
      try {
        const deleteRes = await page.request.delete(`${cleanupContext.apiBase}/api/v1/nodes/${cleanupContext.id}`, {
          headers: { Authorization: `Bearer ${cleanupContext.apiToken}` },
        });
        if (deleteRes.ok() || deleteRes.status() === 404) {
          log(`Cleanup: completed with status ${deleteRes.status()}`);
        } else {
          log(`Cleanup: failed with status ${deleteRes.status()}`);
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        log(`Cleanup: failed with error ${message}`);
      }
    }
    await browser.close();
  }
